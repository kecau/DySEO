...
def calc(df, input_word, now_time):
    ...
    sql_input = "INSERT INTO seo_iif_new (keyword, search_date, iif) \
                 VALUES (%s, %s, %s)"
    ...
    for i in range(len(AR_IIF_RESULT)):
        ...
        try:
            temp_iif_keyword = (
                    temp_iif_keyword + AR_IIF_RESULT.iloc[i, 0] + ":" + AR_IIF_RESULT.iloc[i, 1] + "^")
        except:
            pass
        # cursor.execute(sql_input, (AR_IIF_RESULT.iloc[i, 0], now_time, AR_IIF_RESULT.iloc[i, 1], inputword))
        # cursor.execute(sql_input, (AR_IIF_RESULT.iloc[i, 0], now_time, temp_iif_keyword))
        # connection.commit()

    cursor.execute('set names utf8mb4')
    cursor.execute(sql_input, (inputword, now_time, temp_iif_keyword))
    connection.commit()
...
def check_crawling_method():
    crawldatalist = []
    query = "select * from seo_crawl"
    cursor.execute(query)
    select = list(cursor.fetchall())
    print(range(len(select)))

    for i in range(len(select)):
        if select[i][5] % select[i][4] == 0:
            crawldatalist.append(select[i][1])
        else:
            pass

    query = "update seo_crawl SET cycle_check = cycle_check + 1"
    cursor.execute(query)

    crawldatalist_temp = []

    for i in crawldatalist:
        temp_keyword = i.decode('utf8')
        crawldatalist_temp.append(temp_keyword)

    connection.commit()

    return crawldatalist_temp
...
if __name__ == '__main__':
    ...
    inputword_list = check_crawling_method()
    # inputword = input()
    print(inputword_list)
    now_time = datetime.datetime.now()

    for i in range(len(inputword_list)):
        inputword = inputword_list[i]
